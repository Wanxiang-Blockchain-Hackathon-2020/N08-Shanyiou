<template>
  <div class="expenditure-container">
    ...
    <div class="content" v-if="expenditureList.Total">
      <p class="title">支出列表</p>
      <div class="card" v-for="i in expenditureList.Results" :key="i.Id" @click="move(i)">
        <p class="name">{{i.To}}</p>
      ...
      <div class="page">
          ...
          <p>
            <span>{{ page }}</span>/{{Math.ceil(expenditureList.Total / 10)}}
          </p>
...
export default {
  ...
  computed: {
    ...mapState({
      expenditureList: state => state.expenditure.expenditureList,
  ...
  methods: {
    init() {
      ...
      this.$store.dispatch("getExpenditureList", { Offset: (this.page - 1) * this.pageSize , Limit: this.pageSize });
    },
    changePage(page) {
      if(page < 1 || page > (Math.ceil(this.expenditureList.Total / 10))) return;
      this.page = page;
      this.$store.dispatch("getExpenditureList", { Offset: (this.page - 1) * this.pageSize , Limit: this.pageSize });
    },
    move(data) {
      this.$router.push({path:'/expenditure/detail',query: data })
    },
    thousandMark,
  },
  mounted() {
